...
function priorityEnabled(item) {
  return item.riseEnabled !== false;
}

function targetDate(item) {
  return item.plannedDate || item.nextDueDate;
}

function followingEstimate(item) {
  if (!item.plannedDate) return null;
  return addInterval(item.plannedDate, item.intervalValue, item.intervalUnit);
}

function getDueState(item, today = toDateKey()) {
  const diff = calendarDayDiff(today, targetDate(item));
  const hasPlan = Boolean(item.plannedDate);
  if (!priorityEnabled(item)) {
    const subject = hasPlan ? '予定' : '目安';
    const label = diff < 0 ? `${subject}から${Math.abs(diff)}日` : diff === 0 ? `今日が${subject}日` : `${subject}まであと${diff}日`;
    return { key: 'reference', rank: 4, diff, label, hasPlan };
  }
  if (diff < 0) return { key: 'overdue', rank: 0, diff, label: hasPlan ? `予約日から${Math.abs(diff)}日` : `${Math.abs(diff)}日過ぎています`, hasPlan };
  if (diff === 0) return { key: 'today', rank: 1, diff, label: hasPlan ? '今日が予約日です' : '今日です', hasPlan };
  if (diff <= Number(item.riseDays || 0)) return { key: 'soon', rank: 2, diff, label: hasPlan ? `予約まであと${diff}日` : `あと${diff}日`, hasPlan };
  return { key: 'safe', rank: 3, diff, label: hasPlan ? `予約まであと${diff}日` : `あと${diff}日`, hasPlan };
}
...
function sortedItems(items) {
  return [...items].sort((a, b) => {
    ...
    return aState.rank - bState.rank || targetDate(a).localeCompare(targetDate(b)) || a.name.localeCompare(b.name, 'ja');
  });
}
...
function renderSummary() {
  const dueStates = activeItems().filter(priorityEnabled).map((item) => getDueState(item));
...
function itemCard(item) {
  ...
  const referenceBadge = priorityEnabled(item) ? '' : '<span class="reference-badge">目安表示</span>';
...
function updateRiseFields() {
  const enabled = $('#riseEnabled').checked;
  $('#riseDays').disabled = !enabled;
  $('#riseDaysField').classList.toggle('is-disabled', !enabled);
}
...
function openItemForm(item = null) {
  ...
  $('#plannedDate').value = item?.plannedDate || '';
  $('#riseDays').value = item?.riseDays ?? 3;
  $('#riseEnabled').checked = item?.riseEnabled !== false;
...
async function saveItem(event) {
  ...
  const plannedDate = $('#plannedDate').value || null;
  if (!name || !lastCompletedDate || !Number.isInteger(intervalValue) || intervalValue < 1) {
    $('#formError').textContent = '項目名・周期・前回実施日を確認してください。';
    return;
  }
  if (plannedDate && plannedDate < lastCompletedDate) {
    $('#formError').textContent = '次回の予定日は、前回実施日以降の日付にしてください。';
    return;
  }
  const now = new Date().toISOString();
  const item = {
    id: oldItem?.id || uid('item'), name, icon: oldItem?.icon || '✓', category: oldItem?.category || '',
    intervalValue, intervalUnit: $('#intervalUnit').value, lastCompletedDate, plannedDate,
    nextDueDate: addInterval(lastCompletedDate, intervalValue, $('#intervalUnit').value), tab: oldItem?.tab || 'short',
    pinHome: oldItem?.pinHome || false, riseEnabled: $('#riseEnabled').checked, riseDays: Math.max(0, Number($('#riseDays').value) || 0), note: $('#itemNote').value.trim(), private: $('#itemPrivate').checked,
...
async function completeItem(itemId, performedDate = toDateKey()) {
  ...
  const previous = { ...item };
  const updated = { ...item, lastCompletedDate: performedDate, plannedDate: null, nextDueDate: addInterval(performedDate, item.intervalValue, item.intervalUnit), updatedAt: new Date().toISOString() };
  try {
...
async function recalculateItem(itemId) {
  ...
  const plannedDate = item.plannedDate && item.plannedDate >= latest ? item.plannedDate : null;
  return putOne(STORES.items, { ...item, lastCompletedDate: latest, plannedDate, nextDueDate: addInterval(latest, item.intervalValue, item.intervalUnit), updatedAt: new Date().toISOString() });
}
...
function bindEvents() {
  ...
  $('#riseEnabled').addEventListener('change', updateRiseFields);
